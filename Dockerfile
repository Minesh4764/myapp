...
RUN echo "*/1 * * * * root /usr/local/bin/python /app/app.py >> /var/log/testlog.log 2>&1 && s3cmd --access_key=DO00QTR2VTHEK2R9URFJ --secret_key=Qrv4BQUStDQCCQ0wR97WvS0TDtrHGyJwChEet11Cu70 --endpoint-url=https://testpipeline.nyc3.digitaloceanspaces.com put /var/log/testlog.log s3://testpipeline/Dockerlogs/" > /etc/cron.d/my-cron-job
